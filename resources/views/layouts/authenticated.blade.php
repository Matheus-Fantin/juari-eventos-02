...
<head>
    ...
    <title>{{ isset($title) ? $title . ' — Juari Eventos' : 'Painel — Juari Eventos' }}</title>
...
<body class="font-sans text-graphite bg-cream antialiased notranslate min-h-screen flex flex-col">
        ...
        <nav class="max-w-6xl mx-auto flex items-center justify-between px-6 py-4">
            ...
            <ul class="hidden md:flex items-center gap-6 text-sm text-cream/75">
                ...
                @if (auth()->user()?->papel === 'admin')
                    <li><a href="{{ route('admin.testimonials.index') }}" class="hover:text-cream transition {{ request()->routeIs('admin.testimonials.*') ? 'text-cream font-medium' : '' }}">Depoimentos</a></li>
                    <li><a href="{{ route('admin.galleries.index') }}" class="hover:text-cream transition {{ request()->routeIs('admin.galleries.*') ? 'text-cream font-medium' : '' }}">Galeria</a></li>
                @endif
                <li><a href="{{ url('/') }}" target="_blank" class="hover:text-cream transition">Ver site ↗</a></li>
            </ul>
    ...
    <footer class="border-t border-graphite/10">
        <div class="max-w-6xl mx-auto px-6 py-5 text-xs text-graphite/50">
            &copy; {{ date('Y') }} Juari Eventos — Painel administrativo
        </div>
    </footer>
