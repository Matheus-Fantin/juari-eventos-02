        ...
        @forelse ($depoimentos as $d)
            <div class="rounded-xl border border-graphite/10 bg-white p-6 flex flex-col sm:flex-row sm:items-start gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-xs text-graphite/50">{{ $d->autor }} — {{ $d->evento_tipo }} · {{ $d->created_at->format('d/m/Y') }}</p>
                </div>

                <div class="flex sm:flex-col gap-2 shrink-0">
                    @if ($d->publicado)
                        <form method="POST" action="{{ route('admin.testimonials.unpublish', $d) }}">
                            @csrf @method('PATCH')
                    ...
                    @else
                        <form method="POST" action="{{ route('admin.testimonials.approve', $d) }}">
                            @csrf @method('PATCH')
                    ...
                    @endif
                    <form method="POST" action="{{ route('admin.testimonials.destroy', $d) }}"
